...
class PortfolioSummaryInstance:  # pragma: no cover
    ...
    def _process_node(self, node, dividends):
        ...
        balance = cost_or_value(node.balance, "at_value", g.ledger.prices._map, date=date)
        cost = cost_or_value(node.balance, "at_cost", g.ledger.prices._map, date=date)
        #### ADD Units to the report
        units = cost_or_value(node.balance, "units", g.ledger.prices._map, date=date)
        ### Get row currency
        row_currency = None
